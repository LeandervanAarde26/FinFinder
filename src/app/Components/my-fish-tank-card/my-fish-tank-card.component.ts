...
export class MyFishTankCardComponent {
  ...
  constructor(private buildService: PrebuildsService, private cdr: ChangeDetectorRef, private router: Router, private route: ActivatedRoute){}
  ...
  deleteBuild(id: string){
    ...
    this.buildService.deleteUserBuild(id).subscribe({
      next: data =>{
        ...
        this.router.navigate([`/dashboard/builds`],{relativeTo: this.route, queryParamsHandling: 'preserve' })
